package Homework_240411;

/**
 * 해당 클래스는 Vehicle 클래스를 상속해, 원하는 값을 넣어 주행에 성공하는것을 목표로 하고 있습니다.
 * <p>
 * !!!!!! 해당 문제에서는 Vehicle 클래스를 상속하는 >새 클래스를 선언하는것<과 getVehicle 클래스를 수정하는것만이 허용됩니다.
 * <p>
 * 
 * 이 문제는 다음의 조건을 따라야 합니다 :
 * - 자신만의 새 클래스를 생성합니다. 새 클래스는 Vehicle을 상속받아야 합니다. (1) 새클래스 생성 후 상속받기
 * - 구현된 Vehicle을 상속한 클래스를 getVehicle 메서드가 반환하도록 수정해야 합니다. (2) 새클래스 이름으로 반환
...
public class ModularExample {
    ...
    //새로운 클래스를 만드는데 이제 Vehicle 상속하는...
    static class A extends Vehicle {
        ...
        } //아래서 복사붙 (얘 없으면 오류남. 이유는 모르겠음..)
    }

    //수정할 수 있는 코드
    // 여기서 새클래스 이름 반환
    public static Vehicle getVehicle() {
        return new A(); //(2) 새클래스 이름으로 반환
    }
    ...
    static abstract class Vehicle {
        ...
        public abstract void onTick(int currentTick, int fuel);// 복사해서 새클래스에 넣기
    }
